...
def save_checkpoint(save_model, i_iter, args, is_best=True):
    ...
    save_best_checkpoint(dict_model, is_best, args.snapshot_dir)


def train(model, dg_source_train, dg_target_train, dg_target_semi_train, dg_test, optimizer, args):
    ...
    model.train()
    is_best = False
    weight = torch.autograd.Variable(torch.Tensor([0.16, 0.71, 0.13]))
    ...
    for e_ in range(args.epoch):
        ...
        for idx in range(loops):
            ...
            # for target
            sent_vecs, mask_vecs, label_list, sent_lens, texts, _, _ = next(dg_target_semi_train.get_ids_samples())
            output1, output2, output3, f_loss = model(sent_vecs.cuda(), mask_vecs.cuda(),
                             label_list.cuda(), sent_lens.cuda())
            cls_loss = criterion_cls(output1, label_list.cuda()) + criterion_cls(output2, label_list.cuda())
            cls_loss /= 2
            ...
            output1_softmax = torch.nn.functional.softmax(output1)
            output2_softmax = torch.nn.functional.softmax(output2)

            #class balance
            loss_balance = util.robust_binary_crossentropy(torch.mean(output1_softmax, dim=0), weight.cuda()).mean()
            loss_balance += util.robust_binary_crossentropy(torch.mean(output2_softmax, dim=0), weight.cuda()).mean()
            loss_balance /= 2



            output1_logit, output1_label = torch.max(output1_softmax, dim=1)
            output2_logit, output2_label = torch.max(output2_softmax, dim=1)
            ...
            if selected_label.size(0) > 0:
                ...
                loss = unsupervised_loss * args.lambda_unsupervised + loss_balance * args.lambda_balance
                model.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip_norm, norm_type=2)
...
def evaluate_test(dr_test, model, args, sample_out=False):
    ...
    while dr_test.index < dr_test.data_len:
        ...
        output1, output2, output3 = model.predict(sent.cuda(), mask.cuda(), sent_len.cuda())
...
def main():
    ...
    target_semi_data = dr.load_data(args.target_semi_path)
    #source_train_data.extend(target_semi_data)
    ...
    dg_target_semi_train = data_generator(args, target_semi_data)
    ...
    if args.training:
        train(model, dg_source_train, dg_target_train, dg_target_semi_train, dg_test, optimizer, args)
    else:
        pass
