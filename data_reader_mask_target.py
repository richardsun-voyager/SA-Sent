...
class dataHelper():
    ...
    def process_raw_data(self, data, stanford_tokenizer=False):
        ...
        for sent_i in np.arange(sent_len):
            ...
            for opi_i in np.arange(opi_len):
                ...
                #Replace the target with a special token TTTTT
                text = sent_inst.text
                text = re.sub(target, 'TTTTT', text)
                #Tokenize the sentence
                if stanford_tokenizer:
                    sent_tokens = self.stanford_tokenize(text)
                else:
                    sent_tokens = self.tokenize(text)
                ...
                target_index = sent_tokens.index('TTTTT')
                mask[target_index] = 1
                ...
                #Record label
                class_ind = self.label2id[label]
                
                #Record all the information
                target_text_items.append([text, target, sent_tokens, mask, class_ind])
            #print('Raw data tokenized')

        return target_text_items, text_words
    ...
    def text2ids(self, data, word2id, is_training):
        ...
        for item in data:
            ...
            target_text_items.append([text, target, sent_tokens, mask, sent_ids, class_ind])

        return target_text_items
    ...
    def to_batches(self, data, if_batch = False):
        ...
        for item in data:
            text, target, sent_tokens, mask, sent_ids, class_ind = item

            if sent_tokens is None or mask is None or class_ind is None: 
                continue
            all_triples.append([sent_tokens, mask, class_ind, sent_ids, str(text), target, None])
            pair_couter[class_ind] += 1
