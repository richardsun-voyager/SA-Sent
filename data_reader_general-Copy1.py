...
class data_generator:
    ...
    def pad_char_data(self, sents, masks, labels, tokens, targets, target_ids):
        '''
        Padding sentences to same size
    ...
    def map_char_ids(self, tokens, char_id):
        '''
        ...
        def word2char(word):
            return list(map(chard2id, word))

        def sent2char(text, chard2id):
            words = tokenize(text)
            lens = list(map(len, words))
            sent_ids = list(map(word2char, words))
