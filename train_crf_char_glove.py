...
def train(model, dg_train, dg_valid, dg_test, optimizer, args, tb_logger):
    ...
    for e_ in range(args.epoch):
        ...
        for idx in range(loops):
            ...
            sent_ids, mask_vecs, label_list, sent_lens, batch_char_ids_tensor, batch_char_lens_tensor, _ = next(dg_train.get_char_ids_samples())
            if args.if_gpu:
                sent_ids, mask_vecs = sent_ids.cuda(), mask_vecs.cuda()
                label_list, sent_lens = label_list.cuda(), sent_lens.cuda()
                batch_char_ids_tensor = batch_char_ids_tensor.cuda()
                batch_char_lens_tensor = batch_char_lens_tensor.cuda()
            cls_loss, norm_pen = model(sent_ids, mask_vecs, label_list, sent_lens, 
                                       batch_char_ids_tensor, batch_char_lens_tensor)
...
def evaluate_test(dr_test, model, args, sample_out=False):
    ...
    while dr_test.index < dr_test.data_len:
        sent_ids, mask_vecs, label_list, sent_lens, batch_char_ids_tensor, batch_char_lens_tensor, _ = next(dr_test.get_char_ids_samples())
        if args.if_gpu:
            sent_ids, mask_vecs = sent_ids.cuda(), mask_vecs.cuda()
            label_list, sent_lens = label_list.cuda(), sent_lens.cuda()
            batch_char_ids_tensor = batch_char_ids_tensor.cuda()
            batch_char_lens_tensor = batch_char_lens_tensor.cuda()
        pred_label, best_seq = model.predict(sent_ids, mask_vecs, sent_lens, batch_char_ids_tensor, batch_char_lens_tensor)
        

        #Compute correct predictions
        correct_count += sum(pred_label==label_list).item()
        
        true_labels.extend(label_list.cpu().numpy())
        pred_labels.extend(pred_label.cpu().numpy())
        
        ##Output wrong samples, for debugging
        indices = torch.nonzero(pred_label!=label_list)
        # if len(indices) > 0:
        #     indices = indices.squeeze(1)
        # if sample_out:
        #     with open(mistake_samples, 'a') as f:
        #         for i in indices:
        #             line = texts[i] + '###' + ' '.join(targets[i]) + '###' + str(label_list[i]) + '###' + str(pred_label[i]) + '\n'
        #             f.write(line)
