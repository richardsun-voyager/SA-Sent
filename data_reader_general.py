...
class data_generator:
    def __init__(self, config, data_batch, is_training=True):
        ...
        ##Char level
        self.char_id = None
        with open('data/tweets/vocab/char_dict', 'rb') as f:
            char_id, id_char, chars = pickle.load(f)
    ...
    def pad_word_data(self, sents, masks, labels, tokens, targets, target_ids):
        '''
        Padding sentences to same size
        sents: sequence of word ids
        masks: target mask
        labels: labels
        tokens: words
        targets:targets
        ...
        return sent_ids, mask_vecs, label_list, sent_lens, tokens, targets, target_ids 
    
    def chard2id(self, c):
        try:
            ids = self.char_id[c]
        except:
            ids = len(self.char_id) + 1
        return ids

    def word2char(self, word):
        return list(map(self.chard2id, word))

    def sent2char(self, sent_words):
        lens = list(map(len, sent_words))#word length
        sent_ids = list(map(self.word2char, sent_words))#map each word into character ids
        return sent_ids, lens
    
    def map_char_ids(self, tokens):
        '''
        tokens: a batch of word sequence, [['I', 'like', 'apples'], ['So', 'good']]
        char_id: dictionary
        '''

        #with Pool(10) as p:
            #results = p.map(self.sent2char, tokens)
        results = map(self.sent2char, tokens)
        batch_char_ids, batch_char_lens = zip(*results)
        return batch_char_ids, batch_char_lens
    
    def pad_char_data(self, batch_char_ids, batch_char_lens):
        '''
        Turn the char list into tensor
        batch_char_ids: batch_size, word_len, char_len
        batch_char_lens: word_len
        '''
        max_char_len = 30
        max_word_num = max(map(len, batch_char_lens))
        word_num = list(map(len, batch_char_lens))
        batch_char_ids_tensor = torch.full((len(batch_char_ids), max_word_num, max_char_len), len(self.char_id) + 1)
        batch_char_lens_tensor = torch.full((len(batch_char_ids), max_word_num), 0)
        batch_char_ids_tensor = batch_char_ids_tensor.long()
        batch_char_lens_tensor = batch_char_lens_tensor.long()
        for i in range(len(batch_char_ids)):
            for j in range(word_num[i]):
                batch_char_ids_tensor[i, j, :batch_char_lens[i][j]] = torch.LongTensor(batch_char_ids[i][j])
                batch_char_lens_tensor[i, j] = batch_char_lens[i][j]
        return batch_char_ids_tensor, batch_char_lens_tensor
        
        
            
       
    
    def get_char_ids_samples(self):
        '''
        Map words into a sequence of characters
        '''
        if self.is_training:
        ...
        sent_ids, mask_vecs, label_list, sent_lens, tokens, targets, target_ids = self.pad_word_data(token_ids,
                                                                                                   mask_list, 
                                                                                                   label_list, 
                                                                                                   tokens, targets, 
                                                                                                   target_ids)
        #if a word is too long, cut it to 30 characters
        batch_words = []
        for token in tokens:
            token = [w if len(w)<=30 else w[:30] for w in token]
            batch_words.append(token)
        
        batch_char_ids, batch_char_lens = self.map_char_ids(batch_words)
        batch_char_ids_tensor, batch_char_lens_tensor = self.pad_char_data(batch_char_ids, batch_char_lens)
        yield sent_ids, mask_vecs, label_list, sent_lens, batch_char_ids_tensor, batch_char_lens_tensor, tokens
