...
parser.add_argument('--config', default='cfgs/config_parse_gcnn_glove.yaml')
...
def train(model, dg_train, dg_valid, dg_test, optimizer, args):
    ...
    for e_ in range(args.epoch):
        ...
        for idx in range(loops):
            ...
            cls_loss = model(sent_vecs.cuda(), mask_vecs.cuda(), label_list.cuda(), sent_lens.cuda(), texts)
        ...
        #Only output testing accuracy if valid_acc increases
...
def evaluate_test(dr_test, model, args, sample_out=False):
    ...
    while dr_test.index < dr_test.data_len:
        ...
        pred_label = model.predict(sent.cuda(), mask.cuda(), sent_len.cuda(), texts)
