...
class biLSTM(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.LSTM(config.embed_dim + config.mask_dim, int(config.l_hidden_size / 2), 
...
class CRF_Pos_AspectSent(nn.Module):
    def __init__(self, config):
        '''
        Input: Glove+Pos+Target embedding
        Model: GRU+Aspect
        '''
        super(CRF_Pos_AspectSent, self).__init__()
        self.config = config

        self.bilstm = biLSTM(config)
        self.feat2tri = nn.Linear(config.l_hidden_size*2, 2)
    ...
    def compute_scores(self, sents, masks, lens, is_training=True):
        ...
        #concatenation model
        context = torch.cat([context, target_emb_avg_exp], 2)
        ...
        for i, tri_score in enumerate(tri_scores):
            ...
            sent_v = torch.mm(select_polarity.unsqueeze(0), context[i, :sent_len, :hidden_dim]) # 1*sen_len, sen_len*hidden_dim=1*hidden_dim
