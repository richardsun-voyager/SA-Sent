...
class CRF_Mem_AspectSent(nn.Module):
    def __init__(self, config):
        '''
        Input: Glove+Pos+Target embedding
        Model: GRU+Aspect
        '''
        super(CRF_Mem_AspectSent, self).__init__()
        self.config = config
    ...
    def compute_scores(self, sents, masks, lens, is_training=True):
        ...
        for _ in range(2):
            ...
            for i, tri_score in enumerate(tri_scores):
                ...
                sent_vs.append(sent_v)
                label_scores.append(label_score)
                select_polarities.append(select_polarity)
                best_latent_seqs.append(best_latent_seq)
                marginals.append(marginal)
            target_vec = torch.cat(sent_vs)#batch_size, hidden_dim
        
            target_vec = target_vec.expand(max_len, batch_size, hidden_dim).transpose(0, 1)
