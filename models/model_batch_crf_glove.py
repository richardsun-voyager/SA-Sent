...
class biLSTM(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.GRU(config.char_dim + config.mask_dim, int(config.l_hidden_size / 2), batch_first=True, num_layers = int(config.l_num_layers / 2),
...
#         context = context.transpose(1, 2)
        ...
        context1 = context + u#Batch_size*max_len*embedding
        #concatenation model
        ...
        ###neural features
        feats = self.feat2tri(context1) #Batch_size*sent_len*2
        ...
        sent_vs = [torch.mm(sp.unsqueeze(0), context1[i, :lens[i], :]) for i, sp in enumerate(select_polarities)]
    ...
    def forward(self, sents, masks, labels, lens, chars, char_lens):
        '''
        ...
        sent_vecs, char_vecs = self.cat_layer(sents, masks, lens, chars, char_lens)
        scores, s_prob  = self.compute_scores(char_vecs, masks, lens)
    ...
    def predict(self, sents, masks, lens, chars, char_lens):
        if self.config.if_reset:  self.cat_layer.reset_binary()
        sent_vecs, char_vecs = self.cat_layer(sents, masks, lens, chars, char_lens)
        scores, best_seqs = self.compute_scores(char_vecs, masks, lens, False)
