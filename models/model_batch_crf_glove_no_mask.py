######No mask embedding in this model#########################
#########No mask embedding in this model########################################
import torch
...
class biLSTM(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.GRU(config.embed_dim , int(config.l_hidden_size / 2), batch_first=True, num_layers = int(config.l_num_layers / 2),
            bidirectional=True)
        init_ortho(self.rnn)
...
class AspectSent(nn.Module):
    def __init__(self, config):
        ...
        kernel_num = int(config.l_hidden_size/2)
    ...
    def forward(self, sents, masks, labels, lens):
        ...
        sents = self.cat_layer(sents, masks)[:,:,:-30]
