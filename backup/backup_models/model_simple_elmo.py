...
class attTSA(nn.Module):
    ...
    def compute_score(self, sent, mask, lens):
        ...
        mask: a list of mask  for target index, batch_size*sent_len
        label: a list labels
        '''
        ...
        context = sent#Batch_size*sent_len*hidden_dim
        #Get the indice of tatgets


        mask = mask.type_as(sent)
        #We hope that LSTM can capture information for the target word, and the context embedding of that target word is enough to support classification
        #Expand mask, batch_size*sent_len*hidden_dim
        mask_exp = mask.expand(self.config.embed_dim, batch_size, sent_len).transpose(0, 1).transpose(1, 2)
        target_output = context * mask_exp
        target_output = torch.sum(target_output, 1)/torch.sum(mask_exp, 1)#batch_size*hidden_dim
        
        #Dropout
        sents_vec = target_output
        if self.training:
            sents_vec = self.dropout(sents_vec)

        output = self.vec2label(sents_vec)#Bach_size*label_size

        logits = F.softmax(output, dim=1)#Batch_size*label_size
    ...
    def forward(self, sent, mask, label, lens):
        ...
        scores, _ = self.compute_score(sent, mask, lens)
    ...
    def predict(self, sent, mask, sent_len):
        ...
        scores, logits = self.compute_score(sent, mask, sent_len)
