...
class CNN_Gate_Aspect_Text(nn.Module):
    ...
    def compute_score(self, sents, masks, lens, weights):
        '''
        inputs are list of list for the convenince of top CRF
        Args:
        sents: a list of sentences， batch_size*max_len*emb_dim
        masks: a list of binry to indicate target position, batch_size*max_len
        weights: btach_size*max_len
        '''
        ...
        weights = weights.expand(dim, batch_size, sent_len).transpose(0, 1).transpose(1, 2)#batch_size, sent_len, hidden_dim
    ...
    def forward(self, sents, masks, labels, lens, weights):
        #Sent emb_dim + 50
        
        sent = F.dropout(sents, p=0.2, training=self.training)
        scores = self.compute_score(sents, masks, lens, weights)
        loss = nn.NLLLoss()
    ...
    def predict(self, sents, masks, sent_lens, weights):
        #sent = self.cat_layer(sent, mask)
        scores = self.compute_score(sents, masks, sent_lens, weights)
