...
class attTSA(nn.Module):
    def __init__(self, config):
        ...
        self.hidden2hidden = nn.Linear(config.l_hidden_size, config.l_hidden_size)
        self.target2hidden = nn.Linear(config.embed_dim, config.l_hidden_size)
    ...
    def compute_score(self, sent, target, lens):
        ...
        #reduce dimension
        target_proj = self.target2hidden(target)#Batch_size*hidden_dim
        ...
        context_proj = self.hidden2hidden(context)#Batch_size*sent_len*hidden_dim
        target_proj = target_proj.expand(sent_len, batch_size, self.config.l_hidden_size).transpose(0, 1)
        #Padding zeros for the short sentences
        context_target = torch.cat([context_proj, target_proj], 2)
        pack = nn_utils.rnn.pack_padded_sequence(context_target, 
                                                 lens, batch_first=True)
        #Batch_size*sent_len*(2*hidden_dim)
        context_target, _ = nn_utils.rnn.pad_packed_sequence(pack, batch_first=True)
        context_target = F.tanh(context_target)
        #batch_size*sent_len*1
        alpha = F.softmax(self.concatvec_linear(context_target), 1)
        #batch_size*1*sent_len
        alpha = alpha.transpose(1, 2)

        sents_vec = torch.bmm(alpha, context).squeeze(1)#Batch_size*hidden_dim
    ...
    def predict(self, sent, target, sent_len):
        ...
        scores, logits = self.compute_score(sent, target, sent_len)
